...
class SD(Module,AutoCSR):
    ...
    ##Valores Internos        
        ...
        self.EnableDataReadRegisterCSR  = CSRStatus()
        self.BussyDataWriteRegisterCSR  = CSRStatus()
        self.InputDataRegister          = Signal(8)
        self.EnableDataReadRegister     = Signal()
        self.BussyDataWriteRegister     = Signal()
    ...
    ##Instancia
        ...
        self.specials +=Instance("SD",
            ...
            o_EnableDataReadRegister    = self.EnableDataReadRegister,
            o_InputDataRegister         = self.InputDataRegister,   
            i_EnableDataWriteRegister   = self.EnableDataWriteRegister.storage,
            i_OuputDataRegister         = self.OuputDataRegister.storage, 
            o_BussyDataWriteRegister    = self.BussyDataWriteRegister,
            i_SPI_EnableRegister        = self.SPI_EnableRegister.storage 
        )
        self.comb +=[
            self.InputDataRegister.eq(self.InputDataRegisterCSR.status),
            self.EnableDataReadRegister.eq(self.EnableDataReadRegisterCSR.status),
            self.BussyDataWriteRegister.eq(self.BussyDataWriteRegisterCSR.status)
        ]
